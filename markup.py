...
def markup_main():
    ...
    item1 = types.KeyboardButton("✏️ Написати запит")
...
def markup_agent():
    ...
    item1 = types.InlineKeyboardButton("❗️ Чекаює відповідь від підтримки", callback_data='waiting_reqs:1')
    item2 = types.InlineKeyboardButton("⏳ Чекає відповідь від користувача", callback_data='answered_reqs:1')
    item3 = types.InlineKeyboardButton("✅ Завершені запити", callback_data='confirm_reqs:1')
...
def markup_admin():
    ...
    item1 = types.InlineKeyboardButton("✅ Добавити Агента підтримки", callback_data='add_agent')
    ...
    item4 = types.InlineKeyboardButton("🎲 Згенерувати одноразові паролі", callback_data='generate_passwords')
...
def markup_request_action(req_id, req_status, callback):
    ...
    if req_status == 'confirm':
        ...
        item1 = types.InlineKeyboardButton("✏️ Добавити повідолмення", callback_data=f'add_message:{req_id}:{status_user}')
        item2 = types.InlineKeyboardButton("🗂 Показати файли", callback_data=f'req_files:{req_id}:{callback}:1')

        if status_user == 'user':
            item3 = types.InlineKeyboardButton("✅ Завершити запит", callback_data=f'confirm_req:wait:{req_id}')
...
def markup_confirm_req(req_id):
    ...
    item1 = types.InlineKeyboardButton("✅ Подтвердити", callback_data=f'confirm_req:true:{req_id}')
